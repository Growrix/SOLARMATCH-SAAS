...
const ZapIcon = () => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-6 w-6"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2z"/></svg>;
...
const NavItem: React.FC<{ icon: React.ReactNode; label: string; isActive: boolean; onClick: () => void; badgeCount?: number; }> = ({ icon, label, isActive, onClick, badgeCount }) => (
  <button onClick={onClick} className={`relative flex flex-col items-center justify-center space-y-1 w-full pt-2 pb-1 transition-colors duration-200 ${isActive ? 'text-primary' : 'text-slate-500 dark:text-slate-400 hover:text-primary/80'}`}>
    {icon}
...
const InstallerBottomNavBar: React.FC<InstallerBottomNavBarProps> = ({ activePage, setActivePage, onNewBidClick, onMenuClick, currentPage, onHomeClick, onDashboardClick, unreadMessagesCount, newLeadsCount }) => {
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-white dark:bg-black border-t border-gray-200 dark:border-slate-800 shadow-[0_-2px_10px_rgba(0,0,0,0.1)] z-40">
      <div className="flex items-center justify-around h-full max-w-md mx-auto">
        ...
        <NavItem icon={<ZapIcon />} label="Leads" isActive={activePage === 'Lead Feed'} onClick={() => navigateToDashboardPage('Lead Feed')} badgeCount={newLeadsCount} />
        
        <div className="w-16 h-16 flex items-center justify-center">
            <button onClick={onNewBidClick} className="w-14 h-14 bg-primary rounded-full flex items-center justify-center text-white shadow-lg transform -translate-y-4 hover:bg-teal-700 active:scale-95 transition-all" aria-label="Create New Bid">
                <PlusIcon />
            </button>
        </div>

        <NavItem icon={<MessageSquareIcon />} label="Messages" isActive={activePage === 'Messages'} onClick={() => navigateToDashboardPage('Messages')} badgeCount={unreadMessagesCount} />
