...
import ThemeModal from './admin/ThemeModal';
import AdminBottomNavBar from './admin/AdminBottomNavBar';
import AdminMobileSidebarMenu from './admin/AdminMobileSidebarMenu';
...
const PaintbrushIcon = () => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5"><path d="M17 3a2.85 2.85 0 0 0-4 4L7.5 20.5 2 22l1.5-5.5L17 3z"/><path d="m15 5 4 4"/><path d="M22 11.5c0 2-1.5 3.5-3.5 3.5S15 13.5 15 11.5 16.5 8 18.5 8s3.5 1.5 3.5 3.5z"/></svg>;
...
const AdminSidebar: React.FC<{ activePage: string; setActivePage: (page: string) => void; onHomeClick: () => void; onLogoutClick: () => void; }> = ({ activePage, setActivePage, onHomeClick, onLogoutClick }) => {
    const NavItem: React.FC<{ icon: React.ReactNode; title: string; isActive?: boolean; onClick?: () => void; }> = ({ icon, title, isActive, onClick }) => (
        <button onClick={onClick} className={`w-full flex items-center space-x-3 px-4 py-2.5 rounded-lg transition-colors text-sm font-medium ${ isActive ? 'bg-primary/10 text-primary dark:bg-primary/20' : 'text-slate-500 dark:text-slate-400 hover:bg-gray-200 dark:hover:bg-slate-800'}`}>
          {icon}
          <span>{title}</span>
        </button>
    );
    ...
    return (
        <aside className="dashboard-sidebar w-64 flex-shrink-0 border-r border-gray-200 dark:border-slate-800 flex flex-col p-4 h-full">
            ...
            <nav className="flex-grow space-y-1">
                <NavItem icon={<LayoutDashboardIcon />} title="Dashboard" onClick={() => setActivePage('Dashboard')} isActive={activePage === 'Dashboard'} />
                <NavItem icon={<UsersIcon />} title="User Management" onClick={() => setActivePage('User Management')} isActive={activePage === 'User Management'} />
                <NavItem icon={<FileTextIcon />} title="Content Management" onClick={() => setActivePage('Content Management')} isActive={activePage === 'Content Management'}/>
                <NavItem icon={<PaintbrushIcon />} title="Theme Settings" onClick={() => setActivePage('Theme Settings')} isActive={activePage === 'Theme Settings'} />
                <NavItem icon={<SettingsIcon />} title="Global Settings" onClick={() => setActivePage('Global Settings')} isActive={activePage === 'Global Settings'}/>
            </nav>
...
const AdminHeader: React.FC<{ onHomeClick: () => void; onMenuClick: () => void; pageTitle: string; theme: Theme; setTheme: (theme: Theme) => void; }> = ({ onHomeClick, onMenuClick, pageTitle, theme, setTheme }) => (
    <header className="glass-header h-20 flex-shrink-0 flex items-center justify-between px-4 sm:px-8">
        <div className="flex items-center space-x-4">
             <button onClick={onMenuClick} className="md:hidden p-2 -ml-2 text-slate-500 dark:text-slate-400">
                <MenuIcon />
             </button>
        ...
        <div className="flex items-center space-x-4">
            ...
            <button onClick={onHomeClick} className="text-sm font-semibold text-primary hover:text-teal-700">Back to Main Site</button>
        </div>
    </header>
);
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onHomeClick, onLogoutClick, theme, setTheme, isHeaderVisible }) => {
    ...
    const renderContent = () => {
                ...
                <div className="max-w-4xl mx-auto animate-fade-in space-y-6">
                    ...
                    <div className="theme-card">
                         <div className="p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                            <div>
                                <h3 className="text-lg font-semibold text-slate-900 dark:text-white flex items-center gap-2"><PaintbrushIcon />Theme & Branding</h3>
                                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Customize the look and feel of your site.</p>
                            </div>
                            <button 
        ...
        if (activePage === 'Theme Settings') {
             return <PlaceholderContent title="Theme Settings (Modal Opened)" />;
        }
    ...
    return (
        <div className="homeowner-dashboard-bg min-h-screen text-slate-800 dark:text-slate-200 animate-fade-in">
            ...
            <div className="md:pl-64">
                <div className="flex flex-col min-h-screen">
                    <div className={`sticky top-0 z-20 transition-transform duration-300 ${isHeaderVisible ? 'translate-y-0' : '-translate-y-full'}`}>
                      <AdminHeader onHomeClick={onHomeClick} pageTitle={activePage} onMenuClick={() => setIsMobileMenuOpen(true)} theme={theme} setTheme={setTheme} />
                    </div>
            ...
            <div className="hidden md:fixed md:inset-y-0 md:left-0 md:z-40 md:flex">
                <AdminSidebar activePage={activePage} setActivePage={handleSetActivePage} onHomeClick={onHomeClick} onLogoutClick={onLogoutClick} />
            </div>
