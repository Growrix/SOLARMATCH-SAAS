...
const QuoteBuilderModal: React.FC<QuoteBuilderModalProps> = ({ isOpen, onClose, lead, onSubmitQuote }) => {
  ...
  const inputClasses = "w-full px-4 py-3 bg-background rounded-xl shadow-neu-inset border border-border/50 focus:outline-none focus:shadow-neu-inset-sm focus:border-primary/50 text-foreground transition-all";
  const selectClasses = "w-full px-4 py-3 bg-background rounded-xl shadow-neu-inset border border-border/50 focus:outline-none focus:shadow-neu-inset-sm focus:border-primary/50 text-foreground transition-all appearance-none";
      ...
      <div ref={modalRef} className="bg-background relative w-full h-full md:max-w-7xl md:h-[95vh] md:rounded-2xl flex flex-col animate-scale-in shadow-neu-outset-lg" onClick={e => e.stopPropagation()}>
        ...
        <header className="flex-shrink-0 p-4 border-b border-border flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-foreground">Quote Builder: {lead.name}</h2>
        ...
        <div className="flex-grow flex flex-col md:flex-row overflow-hidden">
          ...
          <div className={`flex-grow p-4 overflow-y-auto space-y-6 ${mobileTab === 'preview' ? 'hidden md:block' : ''} md:w-2/3`}>
            ...
            <div className="bg-background rounded-2xl shadow-neu-inset p-4">
                <h3 className="text-sm font-semibold mb-2 text-foreground">Quick Presets</h3>
                <div className="flex gap-2">{MOCK_PRESETS.map(p => <Button key={p.name} onClick={() => applyPreset(p.name)} variant="secondary" className="flex-1 text-sm px-2 py-1">{p.name}</Button>)}</div>
            </div>
            {/* System Design */}
            <div className="bg-background rounded-2xl shadow-neu-inset p-4">
              <h3 className="text-sm font-semibold mb-2 flex items-center gap-2 text-foreground"><Zap className="h-4 w-4" /> System Design</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div><label className="text-xs text-muted-foreground block mb-1">System Size (kW)</label><input type="number" value={quoteData.systemSize} onChange={e => setQuoteData(p => ({...p, systemSize: parseFloat(e.target.value)}))} className={inputClasses} /></div>
                <div><label className="text-xs text-muted-foreground block mb-1">Panel Model</label><select value={quoteData.panelId} onChange={e => setQuoteData(p => ({...p, panelId: e.target.value}))} className={selectClasses}>{MOCK_PANEL_MODELS.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}</select></div>
                <div><label className="text-xs text-muted-foreground block mb-1">Inverter Model</label><select value={quoteData.inverterId} onChange={e => setQuoteData(p => ({...p, inverterId: e.target.value}))} className={selectClasses}>{MOCK_INVERTER_MODELS.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}</select></div>
                <div><label className="text-xs text-muted-foreground block mb-1">Battery</label><select value={quoteData.batteryId ?? ''} onChange={e => setQuoteData(p => ({...p, batteryId: e.target.value || null}))} className={selectClasses}><option value="">None</option>{MOCK_BATTERY_MODELS.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}</select></div>
              </div>
            </div>
            {/* Itemized Pricing */}
            <div className="bg-background rounded-2xl shadow-neu-inset p-4">
                <h3 className="text-sm font-semibold mb-2 flex items-center gap-2 text-foreground"><DollarSign className="h-4 w-4" /> Itemized Pricing</h3>
                <div className="space-y-2">
                    {quoteData.lineItems.map(item => (
                        <div key={item.id} className="grid grid-cols-12 gap-2 items-center">
                            ...
                            <div className="col-span-2 text-right text-sm font-semibold text-foreground">{`$${(item.qty * item.unitPrice).toLocaleString()}`}</div>
                ...
                <Button onClick={addLineItem} variant="minimal" className="mt-2 text-sm px-3 py-1.5">
          ...
          <div className={`flex-shrink-0 p-4 overflow-y-auto space-y-4 bg-background-alt ${mobileTab === 'editor' ? 'hidden md:block' : ''} md:w-1/3 md:border-l border-border`}>
            ...
            <div className="bg-background rounded-2xl p-4 shadow-neu-inset">
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between font-bold text-base border-t border-border pt-2 mt-2 text-foreground"><span>Total Price</span><span>{`$${calculations.total.toLocaleString()}`}</span></div>
              </div>
            </div>
            {/* Financial Summary */}
            <div className="bg-background rounded-2xl p-4 text-sm shadow-neu-inset">
                <h4 className="font-semibold mb-2 text-foreground">Financial Summary</h4>
                <div className="space-y-1">
                    ...
                    <div className="flex justify-between font-bold text-foreground"><span>Net Cost (est.)</span><span>{`$${calculations.netCost.toLocaleString()}`}</span></div>
                    <div className="flex justify-between text-xs text-muted-foreground pt-2 mt-2 border-t border-border"><span>Price per Watt</span><span>{`$${calculations.pricePerWatt.toFixed(2)} / W`}</span></div>
                    <div className="flex justify-between text-xs text-muted-foreground"><span>Est. Annual Savings</span><span>{`$${calculations.annualSavings.toLocaleString()}`}</span></div>
                    <div className="flex justify-between text-xs text-muted-foreground"><span>Simple Payback</span><span>{`${calculations.payback.toFixed(1)} years`}</span></div>
                </div>
            </div>
          </div>
        </div>
