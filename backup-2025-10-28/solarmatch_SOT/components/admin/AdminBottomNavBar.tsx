...
const PaintbrushIcon = () => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="h-6 w-6"><path d="M17 3a2.85 2.85 0 0 0-4 4L7.5 20.5 2 22l1.5-5.5L17 3z"/><path d="m15 5 4 4"/><path d="M22 11.5c0 2-1.5 3.5-3.5 3.5S15 13.5 15 11.5 16.5 8 18.5 8s3.5 1.5 3.5 3.5z"/></svg>;
...
const NavItem: React.FC<{ icon: React.ReactNode; label: string; isActive: boolean; onClick: () => void; }> = ({ icon, label, isActive, onClick }) => (
  <button onClick={onClick} className={`flex flex-col items-center justify-center space-y-1 w-full pt-2 pb-1 transition-colors duration-200 ${isActive ? 'text-primary' : 'text-slate-500 dark:text-slate-400 hover:text-primary/80'}`}>
    {icon}
    <span className="text-xs font-medium">{label}</span>
  </button>
);

const AdminBottomNavBar: React.FC<AdminBottomNavBarProps> = ({ activePage, setActivePage, onMenuClick, onThemeClick }) => {
  return (
    <div 
    ...
    >
      <div className="grid grid-cols-4 items-center h-full max-w-md mx-auto px-1">
        <NavItem icon={<LayoutDashboardIcon />} label="Dashboard" isActive={activePage === 'Dashboard'} onClick={() => setActivePage('Dashboard')} />
        <NavItem icon={<UsersIcon />} label="Users" isActive={activePage === 'User Management'} onClick={() => setActivePage('User Management')} />
        <NavItem icon={<PaintbrushIcon />} label="Theme" isActive={activePage === 'Theme Settings'} onClick={onThemeClick} />
