...
const NavItem: React.FC<{ icon: React.ReactNode; title: string; isActive: boolean; onClick: () => void; badgeCount?: number; }> = ({ icon, title, isActive, onClick, badgeCount }) => (
    <button onClick={onClick} className={`w-full flex items-center justify-between space-x-3 px-4 py-3 rounded-xl transition-colors text-base font-semibold ${ isActive ? 'bg-primary text-white' : 'bg-gray-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-gray-200 dark:hover:bg-slate-700'}`}>
...
const MobileSidebarMenu: React.FC<MobileSidebarMenuProps> = ({ isOpen, onClose, activePage, setActivePage, onLogoutClick, onMessagesClick, unreadMessagesCount }) => {
        ...
        >
            ...
            >
                ...
                <nav className="flex-grow flex flex-col items-center space-y-3">
                    <NavItem icon={<LayoutDashboardIcon />} title="Dashboard" isActive={activePage === 'Dashboard Overview'} onClick={() => handleNavClick('Dashboard Overview')} />
                    <NavItem icon={<PhoneCallIcon />} title="Call/Visit Quotes" isActive={activePage === 'Call/Visit Quotes'} onClick={() => handleNavClick('Call/Visit Quotes')} />
                    <NavItem icon={<FileSignatureIcon />} title="Written Quotes" isActive={activePage === 'Written Quotes'} onClick={() => handleNavClick('Written Quotes')} />
                    <NavItem icon={<GavelIcon />} title="Bidding Room" isActive={activePage === 'Bidding Room'} onClick={() => handleNavClick('Bidding Room')} />
                    <NavItem icon={<SparklesIcon />} title="AI Insights" isActive={activePage === 'AI Insights'} onClick={() => handleNavClick('AI Insights')} />
                    <NavItem icon={<MessageSquareIcon />} title="Messages" isActive={activePage === 'Messages'} onClick={() => handleNavClick('Messages')} badgeCount={unreadMessagesCount} />
                    <NavItem icon={<UserIcon />} title="My Profile" isActive={activePage === 'My Profile'} onClick={() => handleNavClick('My Profile')} />
                </nav>
