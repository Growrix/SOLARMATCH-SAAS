...
const NavItem: React.FC<{ icon: React.ReactNode; title: string; isActive: boolean; onClick: () => void; }> = ({ icon, title, isActive, onClick }) => (
    <button onClick={onClick} className={`w-full flex items-center justify-center space-x-3 px-4 py-3 rounded-xl transition-colors text-base font-semibold ${ isActive ? 'bg-primary text-white' : 'bg-gray-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-gray-200 dark:hover:bg-slate-700'}`}>
      {icon}
      <span>{title}</span>
    </button>
);

const AdminMobileSidebarMenu: React.FC<AdminMobileSidebarMenuProps> = ({ isOpen, onClose, activePage, setActivePage, onLogoutClick }) => {
        ...
        >
            ...
            >
                <div className="flex items-center justify-between mb-6">
                    ...
                    <button onClick={onClose} className="p-2 -mr-2 rounded-md text-slate-500 dark:text-slate-400 hover:bg-gray-100 dark:hover:bg-slate-800">
                        <XIcon />
                    </button>
                </div>

                <nav className="flex-grow flex flex-col items-center space-y-3">
                    <NavItem icon={<LayoutDashboardIcon />} title="Dashboard" isActive={activePage === 'Dashboard'} onClick={() => handleNavClick('Dashboard')} />
                    <NavItem icon={<UsersIcon />} title="User Management" isActive={activePage === 'User Management'} onClick={() => handleNavClick('User Management')} />
                    <NavItem icon={<FileTextIcon />} title="Content Management" isActive={activePage === 'Content Management'} onClick={() => handleNavClick('Content Management')} />
                    <NavItem icon={<PaintbrushIcon />} title="Theme Settings" isActive={activePage === 'Theme Settings'} onClick={() => handleNavClick('Theme Settings')} />
                    <NavItem icon={<SettingsIcon />} title="Global Settings" isActive={activePage === 'Global Settings'} onClick={() => handleNavClick('Global Settings')} />
                </nav>
