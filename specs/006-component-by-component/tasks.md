...
**Phase 19: Clerk Modal Appearance Investigation & Fix** 🎯 ACTIVE (November 10, 2025)
- 🎯 **Goal**: Investigate and fix Clerk default modal appearance overriding custom neumorphic styling
- 📋 **Approach**: Debug runtime behavior → Verify appearance prop configuration → Test installer redirects
- 🔍 **Scope**: 4 modal components (HomeownerSignup, InstallerSignup, HomeownerSignIn, InstallerSignIn)
- 📄 **Documentation**: `DOC/PHASE-2-ISSUES-AUDIT.md` (existing 485 lines), `DOC/PHASE-3-FIX-REPORT.md` (495 lines)
- 📸 **Evidence**: User screenshot shows Clerk default black modal with Apple/Google buttons (not custom neumorphic design)
- ⏱️ **Estimated**: 2 hours - LOW RISK (code already correct, need runtime investigation)

**Current Status**:
- ✅ Code audit complete: All modals use `routing="virtual"` correctly
- ✅ Catch-all pages confirmed deleted (Phase 3 fix applied)
- ✅ Logout functionality confirmed working (Phase 3 fix applied)
- ❌ Modal appearance issue: Clerk defaults showing despite `appearance` prop configuration
- ⚠️ Installer redirect: User reports redirecting to homeowner dashboard (need to verify)

**Root Cause Hypotheses**:
1. **Appearance Prop Insufficient**: Clerk's `appearance.elements.card: 'bg-transparent shadow-none'` may not fully override internal styling
2. **Z-Index Conflict**: Modal wrapper may be behind Clerk's rendered modal
3. **CSS Specificity**: Global Clerk CSS may override appearance prop with `!important` rules
4. **Race Condition**: Redirect handlers may check role before Clerk metadata syncs

**Investigation Plan**:
1. Run dev server and test actual authentication flow
2. Use browser DevTools to inspect rendered Clerk component
3. Check if `.theme-card` wrapper applies correctly
4. Verify `appearance.elements` selectors match Clerk's actual DOM structure
5. Test installer signup → dashboard redirect end-to-end
6. Review middleware role sync timing

**Subtasks**:
- [ ] 19.1: Start dev server and test homeowner signup flow (10 min)
- [ ] 19.2: Inspect rendered modal with browser DevTools (15 min)
  - Check if wrapper div with `.theme-card` renders
  - Check if Clerk SignUp component renders inside or outside wrapper
  - Check actual CSS classes applied to Clerk elements
  - Document z-index and positioning
- [ ] 19.3: Test all 4 modal flows (signup/signin × homeowner/installer) (20 min)
  - Verify which modals show Clerk defaults vs custom styling
  - Capture screenshots of each modal state
  - Document any differences between modal types
- [ ] 19.4: Test installer redirect flow end-to-end (10 min)
  - Sign up as new installer
  - Verify redirect to `/installer/dashboard` not `/homeowner/dashboard`
  - Check if role metadata syncs before redirect
- [ ] 19.5: Review `appearance` prop configuration in all 4 modals (15 min)
  - Compare current prop structure with Clerk v5 documentation
  - Verify element selectors match Clerk's DOM structure
  - Check if `elements.card`, `elements.rootBox`, `elements.cardBox` all needed
- [ ] 19.6: Implement appearance prop fix (if needed) (20 min)
  - Option A: Add more specific appearance selectors
  - Option B: Add global CSS overrides for Clerk components
  - Option C: Adjust modal wrapper structure
- [ ] 19.7: Implement redirect fix (if needed) (15 min)
  - Simplify LayoutContent handlers to trust `afterSignUpUrl`
  - Remove complex role detection if causing race conditions
  - Verify middleware role sync happens before redirect
- [ ] 19.8: Test fixes across all 3 themes (Dark, Light, Purple) (15 min)
- [ ] 19.9: Run build validation (TypeScript + npm run build) (5 min)
- [ ] 19.10: Document findings and create completion report (10 min)
- [ ] 19.11: Update Phase 19 status in tasks.md (5 min)
- [ ] 19.12: Atomic commit with descriptive message (5 min)

**Estimated Total Time**: 2 hours

**Phase 20: Fix Authentication Role Validation & Redirects** 🚨 CRITICAL (November 10, 2025)
- 🎯 **Goal**: Fix critical security issue where any user can sign into any modal and incorrect dashboard redirects
- 📋 **Approach**: Add role validation to signin modals, fix /api/user/sync, update middleware defaults
- 🔍 **Scope**: 4 modals (signin/signup × homeowner/installer), /api/user/sync, middleware
- 📄 **Documentation**: `DOC/AUTH-REDIRECT-ISSUE-AUDIT.md` (comprehensive root cause analysis)
- 🚨 **Critical Issues**:
  1. ANY user can sign into ANY modal (homeowner can use installer modal, vice versa)
  2. All users redirect to installer dashboard regardless of actual role
  3. No validation that user belongs in the modal they're using
  4. /api/user/sync defaults to HOMEOWNER causing wrong dashboard access
- ⏱️ **Estimated**: 2 hours - HIGH PRIORITY

**Root Cause (from backup comparison)**:
- **OLD (NextAuth)**: Single signin modal, role fetched from DATABASE, no choice of modal type
- **CURRENT (Clerk)**: Separate modals per user type, but NO validation that user matches modal type
- **Problem**: Authentication succeeds based on email/password, but doesn't check if user's DB role matches the modal they're using

**Required Fixes**:
1. Add role validation to signin modals - block wrong user types with error message
2. Fix /api/user/sync to check database FIRST (source of truth)
3. Remove default to HOMEOWNER - return error if role not found
4. Ensure role is set BEFORE any redirects happen

**Subtasks**:
- [ ] 20.1: Update /api/user/sync to check database FIRST (20 min)
  - Change logic: DB lookup → then Clerk fetch → then return error if not found
...
- [ ] 20.2: Add role validation to InstallerSignInModal (15 min)
  - After Clerk authentication, fetch actual role from database
  - If role !== 'INSTALLER', show error: "You have a homeowner account. Please use the homeowner signin."
  - Sign out user and prevent access
  - Only allow installer users to proceed to /installer/dashboard
- [ ] 20.3: Add role validation to HomeownerSignInModal (15 min)
  - After Clerk authentication, fetch actual role from database
  - If role !== 'HOMEOWNER', show error: "You have an installer account. Please use the installer signin."
  - Sign out user and prevent access
  - Only allow homeowner users to proceed to /homeowner/dashboard
- [ ] 20.4: Fix InstallerSignupModal verification redirect (10 min)
  - Ensure role is set in database BEFORE redirect
  - Add webhook sync wait or delay to ensure DB has user record
  - Verify role='INSTALLER' before allowing dashboard access
- [ ] 20.5: Fix HomeownerSignupModal verification redirect (10 min)
  - Ensure role is set in database BEFORE redirect
  - Add webhook sync wait or delay to ensure DB has user record
  - Verify role='HOMEOWNER' before allowing dashboard access
- [ ] 20.6: Update middleware default behavior (15 min)
  - Remove default to HOMEOWNER fallback
  - Redirect to /setup-account or home page if role not found
  - Add comprehensive logging for role check failures
  - Document why user was denied access
- [ ] 20.7: Test all authentication flows (30 min)
  - ✅ Homeowner signup → verify goes to homeowner dashboard only
  - ✅ Installer signup → verify goes to installer dashboard only
  - ❌ Homeowner tries installer signin → blocked with clear error message
  - ❌ Installer tries homeowner signin → blocked with clear error message
  - ✅ OAuth flows maintain role correctly (Google/Apple)
  - ✅ Middleware enforces role-based access correctly
- [ ] 20.8: Documentation and atomic commit (10 min)
  - Update DOC/AUTH-REDIRECT-ISSUE-AUDIT.md with resolution
  - Commit message: "fix(auth): add role validation to signin modals and fix redirect logic"
  - Document test results and verification

**Security Impact**:
- **Before**: ❌ Any user can access any dashboard by choosing the right modal
- **After**: ✅ Only installers can use installer modals, only homeowners can use homeowner modals

**Success Criteria**:
1. Installer CANNOT sign into homeowner modal - shows error
2. Homeowner CANNOT sign into installer modal - shows error
3. Installer signup ALWAYS redirects to installer dashboard
4. Homeowner signup ALWAYS redirects to homeowner dashboard
5. No default HOMEOWNER fallback in any code path
